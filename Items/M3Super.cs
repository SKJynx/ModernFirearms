...
{
	public class M3Super : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Benelli M3");
			Tooltip.SetDefault("Uses 12 gauge shells.");
		}
		...
		public override void SetDefaults()
		{
			...
			item.rare = 2;
			item.crit = 6;
			item.UseSound = mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/M3Shot");
			item.autoReuse = false;
			item.shoot = 10; //idk why but all the guns in the vanilla source have this
			item.shootSpeed = 3f;
			item.useAmmo = mod.ItemType("Shotshells12g");	
		}

        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            int numberProjectiles = 11 + Main.rand.Next(1); // 4 or 5 shots
