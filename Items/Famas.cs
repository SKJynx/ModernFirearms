...
namespace ModernFirearms.Items
{
	public class Famas : ModItem
	{
		public override void SetStaticDefaults()
		{
			...
			Tooltip.SetDefault("Uses 5.56x45 rounds.");
		}
		...
		public override void SetDefaults()
		{
			...
			item.rare = 2;
			item.crit = 6;
			item.UseSound = mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/Famas");
			item.autoReuse = true;
			item.shoot = 10; //idk why but all the guns in the vanilla source have this
			item.shootSpeed = 2.3f;
			item.useAmmo = mod.ItemType("Bullet556");	
		}

		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			Vector2 perturbedSpeed = new Vector2(speedX, speedY).RotatedByRandom(MathHelper.ToRadians(5.6f));
