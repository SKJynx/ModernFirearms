...
namespace ModernFirearms.Items
{
	public class Thompson : ModItem
	{
		...
		{
			DisplayName.SetDefault("Thompson SMG");
			Tooltip.SetDefault("Uses .45 ACP rounds.");
		}
		...
		public override void SetDefaults()
		{
			...
			item.rare = 2;
			item.crit = 3;
			item.UseSound = mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/Thompson");
			item.autoReuse = true;
			item.shoot = 10; //idk why but all the guns in the vanilla source have this
			item.shootSpeed = 2.1f;
			item.useAmmo = mod.ItemType("Bullet45ACP");	
		}

		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			Vector2 perturbedSpeed = new Vector2(speedX, speedY).RotatedByRandom(MathHelper.ToRadians(7.2f));
